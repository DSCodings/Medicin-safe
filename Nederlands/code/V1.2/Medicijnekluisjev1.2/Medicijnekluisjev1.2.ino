...
int gradenservodict = 90;
int gradenservoopen = 15;
int hoelangopen = 30; // hoeveel seconden de deur moet open zijn.



const int buttonboven = 7;  // pins van de knoppen
...
void setup() {
  ...
  MsTimer2::set(1000, countDownOneSecond);  // 1000ms
  ...
  char Card1[] = "9 160 86 211"; // Voegd een nog een kaart toe;
  char Card2[] = "12 177 28 181";
  sleutelsVerpleger.voegSleutelToe(Card1); // voegd een kaart toe voor de verpleger.
  sleutelsPatienten.voegSleutelToe(Card2); // voegd een kaart toe voor de patient.
...
void countDownOneSecond() {
  ...
  updatetime = true; // elke second wordt dan de tijd upgedeat.
...
void opendeur() { // opend de servo deur.
  ...
  s3.write(gradenservoopen); // zet de servo op 90 graden.
}

void scrolldisplay() { // Print wanneer de verpleger/patient de kluis heeft geopend.
  //lcd.clear();
...
String Scroll_LCD_Left(String StrDisplay) { // de wordt naar links verschoven op het display.
  String result;
...
void Clear_Scroll_LCD_Left() { // de positie wordt gereset.
  Li = 16;
  Lii = 0;
}
...
void loop() { // wordt oneindig herhaalt.
  ...
  if (vergrendelingvoortijdstip1 == true && (Tijdenvooropenen.istijdstipgelijk((String(h) + ":" + String(m)  + ":"  + String(s)).c_str()) == true)) { // als de uren en de minuten gelijk zijn aan de ingestelde uren en minuten en de patient kan het kluisje niet eerder openen.
    ...
    heeftmedicijngenomen = false; // de patient heeft geen medicijn genomen.
...
// elke 10 minuten wordt gaat de buzzer aan.
 ...
 if (mopeningtijden < 0){
  ...
  if (maand == 1) { // de maand/jaar/dag wordt automatisch varanderd.
  ...
  else if (maand == 2) { // als het de 2de maand is.  // er wordt ook gekontroleerd op een schrikkel jaar.
  ...
  if (achtergrond1 == true) { // als de achtergrond verlichting aan is.
    if (schermvergrendelingaanuit == false) { // en de scherm bij vergrendeling uit is.
      ...
      lcd.setBacklight(HIGH); // achtergrond verlichting.
    }
  }
  ...
  if ((controldata1 == true || bekijktijden1 == true) && updatetime == true) { // als het menu controle is geopend en timer is geupdate.
    ...
    scrolldisplay(); // scrolt de distplay met de data.
  ...
  if (digitalRead(buttonboven) == LOW) { // als er op de bovenste knop wordt gedrukt.
    ...
    if (setdag == true) { // dag veranderen
    ...
    else if (removerfid == true) { // Als het menu voor de rfid  te verwijderen voor de plegeren
    ...
    else if (addrfid3 == true) { // Als het menu voor de rfid  toe te voegen voor de patient
      ...
      if (achtergrond2 == true) { // als hij wel in het menu is.
        if (achtergrond1 == true) { //  als de achergrond verlichting aan is.
          ...
          achtergrond1 = true; // ze
          lcd.clear(); // print het juiste op de tekst.
          lcd.print("Aan");
        }
      }
    }
    ...
    else if  (schermvergrendeling == true) {// als het menu schem bij vergrendeling er is.
    ...
    if (schermvergrendeling2 == true) { // als het menu voor de scherm bij vergrendeling er is.
      ...
      if (schermvergrendelingaanuit == true) { // als het aan is.
        lcd.print("Uit");
        schermvergrendelingaanuit = false; // zet het uit.
      }
      else { // als het uit is.
        lcd.print("Aan");
        schermvergrendelingaanuit = true; // zet het uit.
      }
    ...
    if (sethopeningstijden == true) { // als de tijd moet gewijzigd worden, telt er eentje bok.
    ...
    if (setmopeningstijden == true) { // als je er een minutje bij wilt.
      mopeningtijden = mopeningtijden  + 1; // telt er 1tje af.
      if (mopeningtijden > 59) { // als de minuten kleiner zijn dan 0 wordt het 59.
        mopeningtijden = 0; // zet het op 59.
        hopeningtijden = hopeningtijden + 1; // telt er een uurtje bij.
      }

    }

    if (seth == true) { // voor er uren bij te tellen.
      h = h + 1;  // zet er een uurtje bij

    }
    if (setm == true) { // voor minuten bij te tellen.
      m = m + 1; // telt er een minutje bij.
    }
  }
  else if (digitalRead(buttonrechts) == LOW) { // de rechte knop.
    ...
    if (addrfid == true) { // als je de rfid wilt toevoegen voor de verpleger.
      ...
      if (sethopeningstijden == true) { // voor de minuten aan te passen.
        ...
        sethopeningstijden = false;
      } else if (setmopeningstijden == true) { // als alles het moet verwijderd worden.
    ...
    else  if (datechange == true) { // als de datum verander wilt veranderen wordt de datum geprint, voor die te veranderen.
      datechange = false; // zet de bools juist.
      datechange2 = true;
      setdag = true;
    ...
    else if (datechange2 == true) { // voor te veranderen van dag naar maan naar jaar.
      lcd.clear();
    ...
    else if (setjaar == true) { // als je een jaartje minder wilt,
    ...
    else if (vergrendelingvoortijdstip == true) { // als je naar het menu voor de openings tijd te verwijderen wilt gaan.
    ...
    else if (verwijderopeneningstijdentoe == true) { // als je naar het menu wilt gaan voor de openingstijden te te voegen.
      verwijderopeneningstijdentoe = false; // zet de bools juist
      voegopeneningstijdentoe = true;
      lcd.clear(); // print het juiste op het scherm.
      lcd.print("Openingstijd");
    ...
    else if (vergrendelingvoortijdstip2 == true) { // als hij moet vergredeldt worden voor het tijdstip wilt veranderen
      if (vergrendelingvoortijdstip1 == true) {
        lcd.clear(); // print het juiste
        lcd.print("Uit");
        vergrendelingvoortijdstip1 = false; // zet de bools juist.
    ...
    if (sethopeningstijden == true) { // als de tijd moet gewijzigd worden, telt er eentje af.
    ...
    if (setm == true) { // als je er een minutje bij wilt.
  ...
  if (rfid.isCard()) {  // kijkt of er een rfid kaart is.
    if (rfid.readCardSerial()) {// leest de rfidkaar nummer
      ...
      if (addrfid2 == true) { // als er een rfid moet worden toegevoegd voor verpleger.
        ...
        lcd.setCursor(0, 1);
        lcd.print("Verpleger");
        sleutelsVerpleger.voegSleutelToe(rfidCard.c_str());// hij voegd de sluetel toe voor de verpleger.
